# example usage
# python scoutingToRaw_gmt.py /eos/user/m/matteomi/scoutingData/run362653/scout_GMT_362653_000506.dat ramdisk/run362653/ 362653 1000
...
def parseMuonScoutingRawFile(infilepath, outdir, rn_override, maxorbits):
  ...
  while True:
    ...
    if (orbitcount_total > maxorbits) | (endGmtOrbit):
    ...
    try:
      ...
      new_ls = orbit >> 18
      new_idx = (orbit >> 12) & 0b111111

      #open a new output file if crossing LS boundary or on first orbit
      if ((new_idx>last_idx) and (new_ls==last_ls)) or ((new_idx==0) and (new_ls>last_ls)):
        ...
        last_ls = new_ls
        last_idx = new_idx
        fout = open(os.path.join(outdir, f'run{rn_override}_ls{str(new_ls).zfill(4)}_index{str(new_idx).zfill(6)}.raw') ,'wb')
        #empty file header, will be updated later
        fout.write(frd_file_header_v2.ver_id)
#       fout.write(bytes(16))
        fout.write(bytes(24))

      read_len = 12*(mAcount+mBcount)
      mu_blk = fin.read(12*(mAcount+mBcount))
